...
좌우 방향키(←, →)를 사용해 함선을 좌우로
이동해보세요.
레이저 (Space bar)
`

    this.currentMissionText = `
‘현재 미션’을 주목하세요! 화면 우측 아래에 표시된
독일어 동사에 어울리는 전치사를 찾아 먹어야 합니다.
정답일 경우 미션이 완료되고, <희망>이 증가합니다.
틀리면? 희망이 줄어들 수 있으니 조심하세요.
`

        this.shieldsText = `
함선의 방어막은 화면 오른쪽에서 확인할 수
있습니다.최대 10칸까지 유지되며, 소행성이나
적의 공격을 맞으면 줄어듭니다.특수 파워업
아이템을 획득하면 방어막이 다시 충전됩니다.
`
...
`
        this.creditsText = `
크레딧은 이 은하에서 가장 중요한 자원입니다.
업그레이드부터 부품 구매까지, 모든 거래는 이걸로
이루어집니다. 전투에서 살아남고 싶다면, 크레딧을
아끼지 마십시오. 함선 아래에 표시되어 있습니다. 
`
        this.resourcesText = `
